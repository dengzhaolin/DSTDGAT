...
adj = get_adjacency_matrix1('../data/PEMSBAY/adj_PEMSBAY.npy')
...
def eval_model(model, valset_loader, criterion):
    model.eval()
    batch_loss_list = []
    for x_batch, y_batch_ in valset_loader:
        x_batch = x_batch.to(DEVICE)
        y_batch = y_batch_[..., :1].to(DEVICE)
...
def predict(model, loader):
    ...
    for x_batch, y_batch_ in loader:
        x_batch = x_batch.to(DEVICE)
        y_batch = y_batch_[..., :1].to(DEVICE)
...
def train_one_epoch(
    ...
    model.train()
    batch_loss_list = []
    for x_batch, y_batch_ in trainset_loader:
        x_batch = x_batch.to(DEVICE)
        y_batch = y_batch_[..., :1].to(DEVICE)
...
if __name__ == "__main__":
    ...
    #model = AGFomer(3,128,1,8,2,12,adj,0.0).to('cuda:0')
