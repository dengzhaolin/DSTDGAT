...
def eval_model(model, valset_loader, criterion):
    ...
    for x_batch, y_batch in valset_loader:
        ...
        out_batch, h_att, query, pos, neg = model(x_batch, y_cov)
        out_batch = SCALER.inverse_transform(out_batch)
        loss2 = separate_loss(query, pos.detach(), neg.detach())
        loss3 = compact_loss(query, pos.detach())
        loss = criterion(out_batch, y_batch)+0.01*loss2*0.01*loss3
...
def train_one_epoch(
    ...
    for x_batch, y_batch in trainset_loader:
        ...
        out_batch,h_att, query, pos, neg = model(x_batch,y_cov,y_batch,batch_seen)
        out_batch = SCALER.inverse_transform(out_batch)
...
if __name__ == "__main__":
    ...
    model=MegaCRN(num_nodes=cfg.get('num_nodes'), input_dim=1, output_dim=1, horizon=12, rnn_units=64)
    # ------------------------------- make log file ------------------------------ #
