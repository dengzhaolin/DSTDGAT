...
def eval_model(model, valset_loader, criterion):
    model.eval()
    batch_loss_list = []
    feat = torch.rand(383552, 3).to(DEVICE)
    for x_batch, y_batch in valset_loader:
        x_batch = x_batch.to(DEVICE)
        y_batch = y_batch.to(DEVICE)

        out_batch = model(x_batch,feat)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_filename', default='pytorch/para_la.yaml', type=str,
    ...
    model=SGODEModel(0.5,**supervisor_config)
